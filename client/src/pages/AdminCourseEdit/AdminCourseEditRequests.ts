...
export const getCourses = async (facId: string): Promise<Course[]> => {
  return new Promise(async (resolve, reject) => {
    try {
      const result = await axios.get("/restricted/courses/" + facId);
      if (result.status === 200) {
...
export const addCourse = async (course: Course): Promise<void> => {
  return new Promise(async (resolve, reject) => {
    try {
      const result = await axios.post("/restricted/course", course);
      if (result.status === 200) {
        resolve();
      } else {
        reject(result.data);
...
export const deleteCourse = async (courseId: string): Promise<void> => {
  return new Promise(async (resolve, reject) => {
    try {
      const result = await axios.delete("/restricted/course/" + courseId);
      if (result.status === 200) {
        resolve();
      } else {
        reject(result.data);
